...
static int spawn_children(pid_t* pids, int* fds, int numchildren, char** args) {
    ...
    for (i = 0; i < numchildren; ++i) {
        ...
        if (pid == CHILD) {
            ...
            int err = errno;
            fprintf(stderr, "Error: Could not spawn program: %s, ", args[0]);
...
static int write_fully(int fd, char* buf, int count) {
    do {
        ...
        if (written < GOOD) {
            int err = errno;
            fprintf(stderr, "Error: Could not write from parent to fd: %d, ", fd);
            fputs(strerror(err), stderr);
            fputc('\n', stderr);
            return written;
        }
...
int main(int argc, char** argv) {
    ...
    for (i = 0; i < numpids; ++i) {
        if (close(fds[i]) < GOOD) {
            int err = errno;
            fprintf(stderr, "Error: Could not close child stdin fd: %d, ", fds[i]);
            fputs(strerror(err), stderr);
            fputc('\n', stderr);
            /* continue anyway */
        }
    ...
    for (i = 0; i < numpids; ++i) {
        ...
        if (waitpid(pids[i], &status, 0) < GOOD) {
            int err = errno;
            fprintf(stderr, "Error: Could not wait for child pid: %d, ", pids[i]);
            fputs(strerror(err), stderr);
            fputc('\n', stderr);
            /* continue anyway */
        }
